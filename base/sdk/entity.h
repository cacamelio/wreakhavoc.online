...
public:
	...
	OFFSET( bool, m_bUseNewAnimstate( ), Displacement::Netvars->m_flLastExoJumpTime + 0x8 );
	OFFSET( CAnimationLayer*, m_AnimationLayers( ), 0x2990u );
	...
	OFFSET( float, m_flLastSetupBonesTime( ), 0x2928u);// - 0x14
	OFFSET( CBoneAccessor, m_BoneAccessor( ), 0x26a4u);// - 0x10
	OFFSET( CStudioHdr*, m_pStudioHdr( ), 0x2950 );// - 0x14
	OFFSET( CBoneMergeCache*, m_pBoneMergeCache( ), 0x2910u );
	OFFSET( VarMapping_t, m_pVarMapping( ), 0x24u );

	OFFSET( CCSGOPlayerAnimState*, m_pAnimState( ), Displacement::Netvars->m_bIsScoped - 0x14 ); // @ida: client.dll @ [8B 8E ? ? ? ? 85 C9 74 3E + 0x2]
	OFFSET( CUtlVector< matrix3x4_t >, m_CachedBoneData( ), 0x2914u ); // DT_BasePlayer::m_CachedBoneData = reinterpret_cast<std::uintptr_t>(core::find_signature(charenc("client.dll"), charenc("8B BF ? ? ? ? 8D 14 49")) + 2); for using base ent
	//OFFSET( CUtlVector< matrix3x4_t >, m_iBoneCount(), 0x2690u );
	...
	float GetLayerSequenceCycleRate( CAnimationLayer* layer, int32_t sequence ) {
		return MEM::CallVFunc<float>( this, 223u, layer, sequence );
	}
	...
	void CalculateIKLocks( float simtime ) {
		return MEM::CallVFunc<void>( this, 192, simtime );
	}
