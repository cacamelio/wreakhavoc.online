...
void FASTCALL Hooks::hkPacketEnd(void* ecx, void* edx) 
{
	...
	// TODO:
	if (ctx.m_iSentCmds.empty() || std::find(ctx.m_iSentCmds.rbegin(), ctx.m_iSentCmds.rend(), Interfaces::ClientState->iCommandAck) == ctx.m_iSentCmds.rend())
		return;
...
void FASTCALL Hooks::hkPacketStart(void* ecx, void* edx, int in_seq, int out_acked) {
	...
	if (ctx.m_iSentCmds.empty() || std::find(ctx.m_iSentCmds.rbegin(), ctx.m_iSentCmds.rend(), out_acked) == ctx.m_iSentCmds.rend())
		return;

	ctx.m_iSentCmds.erase(std::remove_if(ctx.m_iSentCmds.begin(), ctx.m_iSentCmds.end(),
